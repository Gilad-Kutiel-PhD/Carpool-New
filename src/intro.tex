...
ratio is $\half + \inv{2\cmax} - \eps$, for any $\eps>0$.  Given a
parameter $K$, our algorithm starts with the empty carpool matching.
Then, it repeatedly tries to find a better matching by replacing
$t \leq k$ arc in the current solution by $t+1$ arcs that are not in
the solution.
%
